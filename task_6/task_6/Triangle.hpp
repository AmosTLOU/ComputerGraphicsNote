...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if (dotProduct(ray.direction, normal) > 0) //若光线来自物体内部（此时点积结果为正数）
        return inter;
    double u, v, t_tmp = 0;
	...
	// 能走到这一步则说明t、u（b1）、v（b2）均有效，光线与三角形有交点。

	inter.happened = true;
	inter.coords = (1 - u - v)*v0 + u*v1 + v*v2;
	inter.normal = normal;
	inter.distance = sqrtf(dotProduct((ray.origin - inter.coords), (ray.origin - inter.coords)));
	inter.obj = this;
	inter.m = m;
